...
def get_data_by_key_until_end(filename, keyword, end, encoding=""):
	found = False
...
def main():
	...
	if args.end is not None:
		...
		sys.exit("Please select an end")
	if args.encoding is not None:
		encoding = args.encoding
	else:
		encoding = "utf-8"
	if (filename is not None) and (keyword is not None):
		for x in get_data_by_key_until_end(filename, keyword, end, encoding):
			print(x, end="")
